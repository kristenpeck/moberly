#### Modification by Carl Schwarz in 2022-01-07
#### See # CARL comments
...
# CARL source("Db_connect.R")
suppressWarnings(source("Carl_Db_connect.R"))
ls()
...
xtabs(~sex+yr.class, data=catch.history)
...
xtabs(~yr+shoal, data=catch.hist.spawner, exclude=NULL, na.action=na.pass)
#catch.hist.spawner.carl <- catch.hist.spawner

#construct catch histories
# CARL - the catch.hist.spawner has a shoal variable that needs to be ignored when
#        creating the capture history, otherwise you get a separate capture history for id in shoal and id not in shoal
# Suggest you switch to tidyr::pivot_wider which is more flexible
ch.spawner <- catch.hist.spawner %>%  # DO NOT USE AS YOU SEE BELOW
  spread(key=yr, value = tot.catches,fill=0) 
# Notice that because you didn't specify the id variables, "shoal" is used to define a key
# so some fish get 2 histories - such as fish 108
head(ch.spawner)

# Better to use tidyr::pivot_wider() which allows you specify the id columns explicity
ch.spawner <- tidyr::pivot_wider(catch.hist.spawner,
                                 id_cols=c("LTFishIDAutonumber","sex"),
                                 names_from="yr",
                                 names_sort=TRUE,
                                 values_from=freq,
                                 values_fill=0, )
# now notice that fish 108 only has a single history
head(ch.spawner)

# CARL - check this code here -it no longer gets the yearly values
cols <- names(ch.spawner)[5:ncol(ch.spawner)]
cols <- na.omit(names(ch.spawner)[ as.numeric(names(ch.spawner))>= 2008])
...
# Carl - same problem here... the shoal variable will create two histories
# if a fish has different shoal variables
...
# CARL. There are some losses on capture here. You need to create a freq variable that is 1 or -1
ch.spawner$ch <- do.call(paste, c(abs(ch.spawner[cols]),sep=""))
ch.spawner$freq <- 1
ch.spawner$freq[ apply(ch.spawner[cols]<0,1,any)] <- -1
headtail(ch.spawner)
xtabs(~freq, data=ch.spawner, exclude=NULL, na.action=na.pass)
length(cols) #this is the number of events
...
# Carl Create a quadratic term for lambda
moberly.ddl$Lambda
moberly.ddl$Lambda$T2 <- (moberly.ddl$Lambda$Time - mean(moberly.ddl$Lambda$Time))^2 # centre to improve convergence
moberly.ddl$Lambda
...
lambda.Time2=list(formula=~Time+T2)    #populations growth rate varies as a quadratic trend with time
...
pmodel.5=mark(moberly.proc, moberly.ddl, model.parameters=list(Phi=Phi.Time, p=p.time, Lambda=lambda.Time2)) #carl
pmodel.6=mark(moberly.proc, moberly.ddl, model.parameters=list(Phi=Phi.dot,  p=p.time, Lambda=lambda.Time2)) #carl
...
pmodel.5$results$real  # Carl
pmodel.6$results$real  # Carl

rm(pmodel.1,pmodel.2,pmodel.3) # removed model because they were >>2 AIC scores compared to the top model # Carl
