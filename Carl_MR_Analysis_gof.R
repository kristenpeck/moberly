#### CARL  Modified for use by Carl Schwarz
#### See comments # Carl
...
# Carl suppressWarnings(source("Db_connect_CS.R"))
...
# Carl -- need to clean up below code to be consistent with the MR analysis ###########################################
...
# This is the catch.history that matches that in the MR_Analysis file
catch.history <- effort.catch %>% 
  filter(species %in% "LT") %>% 
  ...
  mutate(freq = case_when(fate %in% c("a",NA) ~ 1,
                          fate %in% c("m","m?") ~ -1)) %>%   #assume that suspected deaths are real
  arrange(datetime)
# Carl -- need to clean up above code to be consistent with the MR analysis ###########################################
...
# Carl I suggest you convert this to tidyr::pivot_wider() to avoid problems
